...
async function main(): Promise<void> {
  ...
  const analyzeHandler = createAnalyzeHandler(client, katagoProcess, env.aiRequestTimeoutMs);

  const server = createServer(async (req, res) => {
    try {
      await route(req, res, healthHandler, aiMoveHandler, analyzeHandler);
    } catch (error) {
...
async function route(
  ...
  analyzeHandler: ReturnType<typeof createAnalyzeHandler>,
): Promise<void> {
  ...
  if (method === 'POST' && url.pathname === '/api/ai/analyze') {
